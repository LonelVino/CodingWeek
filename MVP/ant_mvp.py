...
class Ant(object):
        ...
        for i in [-1,0,1]:
            for j in [-1,0,1]:
                ...
                if neighbour_y < 0 or neighbour_y >= num_rows or neighbour_x < 0 or neighbour_x >= num_cols:
                    continue # Neighbour cells outside the map not considered
                ...
                if map[neighbour_y][neighbour_x].type in [Cell.OBSTACLE,Cell.NEST]:
                    continue # Obstacle and nest cells should be skipped
                try:
                    if map[neighbour_y][neighbour_x] in self.history[-4:-1]:
    ...
    def move_ant(self,map):
        ...
        if self.carry_food:
            ...
            self.add_to_history()  
            if self.cell.type == Cell.FOOD:
                self.cell.food_amount -= 1
                self.toggle_carry_food() # Ants pick up food from food cell
                if self.cell.food_amount == 0:
                    self.cell.type = Cell.OTHER
